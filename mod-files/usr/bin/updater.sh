...
menu_reset() {
    ...
    if [[ -f /usr/local/.updatemile ]]; then
      options=("Select version to update to" "Confirm Update (v$(cat /usr/local/.updatemile))" "Exit")
    else
      options=("Select version to update to" "Confirm Update" "Exit")
    fi
    functions=("selectv" "confupd" "quit")
	num_options=${#options[@]}
}
