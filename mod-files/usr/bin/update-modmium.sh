...
menu_reset() {
  ...
  options=("${Y}Update Modmium${N}" "${Y}Change ChromeOS Version${N}" "${B}Change Shell${N}" "${R}Change Source Repository${N}" "${D}Swap Boot Priority${N}" "${Y}Toggle Enrollment${N}" "Add Local Account" "Feature Toggles" "Exit")
