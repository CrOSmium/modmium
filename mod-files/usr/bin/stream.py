...
def find_eocd64(url, file_size):
    ...
    return struct.unpack_from("<Q", eocd64, 48)[0], struct.unpack_from(
        "<Q", eocd64, 40
    )[0]
            ...
            while epos < len(extra) - 4:
                ...
                if tag == 0x0001:
                    ...
                    if uncompressed_size == 0xFFFFFFFF and idx < len(vals):
                        uncompressed_size = vals[idx]
                        idx += 1
                    if compressed_size == 0xFFFFFFFF and idx < len(vals):
                        compressed_size = vals[idx]
                        idx += 1
...
def get_data_offset(url, lh_offset):
    ...
    return (
        lh_offset
        + 30
        + struct.unpack_from("<H", lh, 26)[0]
        + struct.unpack_from("<H", lh, 28)[0]
    )
    ...
    for skip, length, outfile, label in sorted(partitions, key=lambda x: x[0]):
        ...
        with open(outfile, "wb") as f:
            while fetched < length:
                chunk = range_get(
                    url,
                    data_offset + skip + fetched,
                    min(4 * 1024 * 1024, length - fetched),
                )
                f.write(chunk)
                fetched += len(chunk)
                print(
                    f"\r  {label}: {fetched // (1024 * 1024)}MB / {length // (1024 * 1024)}MB",
                    end="",
                    flush=True,
                    file=sys.stderr,
                )
...
def stream_deflate(url, data_offset, partitions):
    ...
    r = requests.get(
        url,
        headers={"Range": f"bytes={data_offset}-", "Accept-Encoding": "identity"},
        stream=True,
    )
    r.raise_for_status()
    ...
    handles = [
        (skip, length, open(outfile, "wb"), label, 0)
        ...
        for chunk in r.iter_content(chunk_size=2 * 1024 * 1024):
            ...
            for skip, length, f, label, written in handles:
                if decompressed + len(out) > skip:
                    ...
                    written += len(data)
                    print(
                        f"\r  {label}: {written // (1024 * 1024)}MB / {length // (1024 * 1024)}MB"
...
def linux_version(kernel_path):
    ...
    try:
        ...
        subprocess.check_output(
            ["binwalk", "--extract", "--directory", d, kernel_path],
...
def tpm_version(kernel_path):
    try:
        out = subprocess.check_output(
            ["futility", "show", kernel_path], stderr=subprocess.DEVNULL
        ).decode()
...
def main():
    ...
    print(
        f"{entry['name']}  {'stored' if ctype == 0 else 'deflate'}  "
        f"{entry['uncompressed_size'] // (1024 * 1024)}MB",
        file=sys.stderr,
    )
    ...
    with tempfile.NamedTemporaryFile(delete=False, suffix=".bin") as f:
        ...
        pt_tmp = f.name
    try:
        fdisk = subprocess.check_output(
            ["fdisk", "-l", pt_tmp], stderr=subprocess.DEVNULL
        ).decode()
    finally:
        os.unlink(pt_tmp)

    def parse_part(name):
        for line in fdisk.splitlines():
            if name in line:
                p = line.split()
    ...
    print(
        f"ROOT-A: {root_sectors * 512 // (1024 * 1024)}MB  KERN-B: {kern_sectors * 512 // (1024 * 1024)}MB",
        file=sys.stderr,
    )
    ...
    if tpm:
        print(f"tpm version: {tpm}")
    if lv:
        print(f"linux version: {lv}")
