...
${G}Edit policies in /usr/share/.policy-test-tool/policies.json${N}
EOF
...
cat > /usr/share/.policy-test-tool/policies.json << EOF
{
  "policy_user": "$email",
  "managed_users": ["*"],
...
Location: /usr/share/.policy-test-tool/policies.json
...
if [[ -f /root/policy.json ]]; then
    ...
    python policy_dump_converter.py \
        --input-dump /root/policy.json \
        --output-policies extracted.json \
        --policy-user $email >/dev/null 2>&1
