...
```
The builder also has a few other flags, which can be seen by running the script with no arguments or passing `--help`.
...
* `-s`, `--bootsplash` (bool)
    * Whether or not to install bootsplashes in in bootsplash/ (requires inkscape).
