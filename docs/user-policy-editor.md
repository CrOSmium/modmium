...
- (Pre-install, optional) Before installing Modmium, enroll and login to your enterprise account, then export json from chrome://policy. Either add `-j /path/to/policies.json` to the flags when building the image, or name the file `policy.json` and place it in `mod-files/root` 
